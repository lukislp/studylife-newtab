...
streak, upcoming sessions, recent activity, course-goal deadlines, programme progress, and recent
notes - one glance, every time you open a tab, no clicking into the app required.
...
And, filling the rest of the screen, a panel grid:

- **Upcoming sessions** - sessions with a `StartTime` still ahead of now (scheduled via the
  calendar or generated by the exam planner), read from `GET /api/sessions`. StudyLife's session
  list is genuinely unbounded and forward-looking, not exclusively past/completed data, so this is
  real scheduled data, not a synthetic stand-in.
- **Recent activity** - your last few *completed* sessions, read from
  `GET /api/sessions/history?onlyCompleted=true` - a real timeline distinct from "upcoming" above.
- **Course goal deadlines** - every open goal with a target date, soonest first (the hero tile
  above only shows the single nearest one).
- **Programme progress** - ECTS earned/total, topics completed/total, and your ECTS-weighted
  average grade if you have one, all from the same `Metrics.GetSummary` call the hero row uses.
- **Recent notes** - your most recently updated notes, title + a one-line preview, read from
  `GET /api/notes`.
...
2. Register a new client:
   ...
   - **Scopes:** `Whoami`, `TimerState.Get`, `Metrics.GetSummary`, `CourseGoals.GetAll`,
     `Sessions.GetAll`, `Sessions.GetHistory`, `Notes.GetAll` (exactly these seven - no write scope)
...
- **Partial failure** - only `Metrics.GetSummary` is load-bearing for the whole page (it feeds the
  hours/streak/programme-progress numbers). If any of `CourseGoals.GetAll`, `TimerState.Get`,
  `Sessions.GetAll`, `Sessions.GetHistory`, or `Notes.GetAll` fails instead, every other panel
  still renders with its own real, current data - only the affected panel shows its own
